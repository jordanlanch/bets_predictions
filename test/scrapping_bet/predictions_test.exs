defmodule ScrappingBet.PredictionsTest do
  ...
  describe "results" do
    ...
    @valid_attrs %{best_bookie: "some best_bookie", best_odds: 120.5, date_game: ~N[2010-04-17 14:00:00], league: "some league", match_title: "some match_title", mean_median: "some mean_median", result_to_bet: "some result_to_bet", sports: "some sports", time_game: ~T[14:00:00], is_finish: false, is_work: false}
    @update_attrs %{best_bookie: "some updated best_bookie", best_odds: 456.7, date_game: ~N[2011-05-18 15:01:01], league: "some updated league", match_title: "some updated match_title", mean_median: "some updated mean_median", result_to_bet: "some updated result_to_bet", sports: "some updated sports", time_game: ~T[15:01:01], is_finish: false, is_work: false}
    @invalid_attrs %{best_bookie: nil, best_odds: nil, date_game: nil, league: nil, match_title: nil, mean_median: nil, result_to_bet: nil, sports: nil, time_game: nil, is_finish: nil, is_work: nil}
    ...
    test "update_or_create_bets/1 returns a bets" do
      bets = bets_fixture()
      assert {:ok, %Bets{} = bets} = Predictions.update_bets(bets, %{best_bookie: "change bookie", best_odds: 50.5, date_game: ~N[2010-04-17 14:00:00], league: "some league", match_title: "some match_title", mean_median: "2.2/2.3", sports: "some sports", time_game: ~T[14:00:00]})
      assert bets.best_bookie == "change bookie"
      assert bets.best_odds == 50.5
      assert bets.date_game == ~N[2010-04-17 14:00:00]
      assert bets.league == "some league"
      assert bets.match_title == "some match_title"
      assert bets.mean_median == "2.2/2.3"
      assert bets.sports == "some sports"
      assert bets.time_game == ~T[14:00:00]
    end
  end
